...
package engine.framework.time
{
    ...
    import engine.framework.debug.Profiler;
    import engine.framework.util.IPrioritizable;
    import engine.framework.util.PBUtil;
    import engine.framework.util.SimplePriorityQueue;
    import engine.framework.util.TypeUtility;
    import engine.framework.util.sprintf;
    ...
    public class TimeManager implements IPBManager
    {
        ...
        {
            if (!started){
				Logger.warn(this, "schedule", "The TimeManager is not started.");
				// Didn't Auto Start
//                start();
			}
...
//                start();
		...
		private var fpsTimes:Vector.<Number> = new Vector.<Number>(10,true);
		private var fpsTimesIndex:int = 0;
		// read-only, public for speed up
        ...
        private function onFrame(event:TimerEvent):void
        {
			...
			// OPTIMIZE
			const frameTime:Number = (currentTime - lastTime);
			fpsTimesIndex = (fpsTimesIndex + 1) % 10;
			fpsTimes[fpsTimesIndex] = frameTime;
			const avgTime:Number = (fpsTimes[0] + fpsTimes[1] + fpsTimes[2] + fpsTimes[3] + fpsTimes[4] 
				+ fpsTimes[5] + fpsTimes[6] + fpsTimes[7] + fpsTimes[8] + fpsTimes[9] ) / 10.0;
			fps = 1000 / avgTime;
			// OPTIMIZE
