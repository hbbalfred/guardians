 ...
 * even simpler, which is why it is in engine.frameworks.util instead of
...
package engine.framework.util
{
    ...
    public class Injector
    {
		 ...
		 * [PBInject] tag over it and that matches the class definition specified
		 ...
		 * property injected into the specified object, make sure that an [PBInject]
        ...
        public static function getInjectionTargets(value:Class):Vector.<InjectionTarget>
        {
            ...
            for each (var node:XML in definition.factory.*.(name() == 'variable' || name() == 'accessor').metadata.(@name == 'PBInject'))
            {
			...
			injectionTargetCache[value] = result;
            return result;
        }
        ...
        public static function getPostConstructTargets(value:Class):Vector.<PostInjectionTarget>
        {
            ...
            for each (var node:XML in definition.factory.*.(name() == 'method').metadata.(@name == 'PBPostInject')) 
            {
			...
			postConstructTargetCache[value] = result;
            return result;
        }
