...
{    
    ...
    public class TypeUtility
    {
        ...
        public static function getFieldType(object:*, field:String):String
        {
			...
			list = typeXML.child("accessor");
			len = list.length();
			for(i = 0; i < len; ++i)
            {
				var property:XML = list[i];
                if (property.attribute("name") == field)
                    return property.attribute("type");
            }
            
            // Look for a matching variable.
			list = typeXML.child("variable");
			len = list.length();
			for(i = 0; i < len; ++i)
            {
				var variable:XML = list[i];
		...
		public static function getListOfPublicFields( object:* ):Array
		{
			...
			var list:XMLList = sourceFields.*;
			var len:int = list.length();
			for(var i:int = 0; i < len; ++i)
			{
				var fieldInfo:XML = list[i];
		...
		public static function getListOfReadonly(object:*):Array
		{
			var fields:Array = [];
			...
			for(var i:int = 0; i < len; ++i)
			{
				...
				var tag:String = fieldInfo.name();
				// Skip anything that is not a field.
				if( tag != "constant" && tag != "accessor")
					continue;
				
				// Skip writeable stuff.
				if( tag == "accessor" && fieldInfo.@access != "readonly")
					continue;
