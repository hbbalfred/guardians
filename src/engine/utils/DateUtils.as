package engine.utils 
{
	...
	{	
		 ...
		 * @param dateStr
		 * @param simple if simple is true , the data string format should be mm/dd/yy
		 * @return
		 */
		public static function utcDateStrToNumber(dateStr:String, simple:Boolean=false):Number
		{
			var dateArr:Array;
			var ret:Number = 0;
			if (simple){
				dateArr = String(dateStr).split("/", 3);
				ret = Date.UTC(parseInt(dateArr[2]), parseInt(dateArr[0])-1, parseInt(dateArr[1]) );
			} else {
				ret = new Date(dateStr).valueOf();
			};
			return (ret);
		}
		...
		/**
		 * Descriptive the Difference of the date
		 * @param __date
		 * @return
		 */
		...
		/**
		 * a easy way to get simple time string 
		 * @param time
		 * @return
		 */
		public static function timeString(time:Number, formatStr:String = "HH:mm:ss" ):String
		{
			if( time <=0 ) 
				time = 0;
			var rest:Date = new Date( time );
			var format:DateTimeFormatter = new DateTimeFormatter("en");
			format.setDateTimePattern( formatStr );
			return format.formatUTC( rest );
		}
	}
} 
